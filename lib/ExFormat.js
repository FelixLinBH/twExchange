...
var cathaybkMapping = {
    '美元(USD)USDollars':'USD',
    '美元現鈔(USD)USDollarCash':'USD',
    '人民幣(CNY)ChineseYuan':'CNY',
    '人民幣現鈔(CNY)ChineseYuanCash':'CNY',
    '港幣(HKD)HongKongDollars':'HKD',
    '港幣現鈔(HKD)HongKongDollarCash':'HKD',
    '英鎊(GBP)PoundSterling':'GBP',
    '瑞士法郎(CHF)SwissFrancs':'CHF',
    '澳大利亞幣(AUD)AustralianDollars':'AUD',
    '新加坡幣(SGD)SingaporeDollars':'SGD',
    '加拿大幣(CAD)CanadianDollars':'CAD',
    '瑞典幣(SEK)SwedishKronor':'SEK',
    '南非幣(ZAR)SouthAfricanRand':'ZAR',
    '日圓(JPY)JapaneseYen':'JPY',
    '日圓現鈔(JPY)JapaneseYenCash':'JPY',
    '丹麥幣(DKK)DanishKroner':'DKK',
    '泰國幣(THB)ThaiBaht':'THB',
    '紐西蘭幣(NZD)NewZealandDollars':'NZD',
    '歐元(EUR)Euro':'EUR',
    '歐元現鈔(EUR)EuroCash':'EUR'
}
...
function cathayFormat(json) {
    ...
    for (var prop in json) {
      if (json.hasOwnProperty(prop)) {
        var searchKey = prop.replace(/　|\r|\n|\t| /g, "");
        var key = cathaybkMapping[searchKey];
        var data = json[prop];

        if (searchKey != "time") {
            var obj = (newJson[key] != undefined)?newJson[key]:{};
            if (searchKey.match(/(現鈔)/)) {
