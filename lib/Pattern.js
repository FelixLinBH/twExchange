...
function cathayPattern($,instance) {   
    $(".table-rate.text-left").children("tbody").children("tr").children("td").each(function (index) {
        var $this = $(this);
        if (index % 3 == 0) {
...
function esunbkPattern($,instance){
    $('table tr').each(function (index) {
        if (index >= 2) {
            ...
            priceObj.cashbuy = (result[4] != undefined)? parseFloat(result[4].trim()).toFixed(5) : "-";
            priceObj.cashsell = (result[5] != undefined)? parseFloat(result[5].trim()).toFixed(5) : "-";
            instance.resultJson[key] = priceObj;
        }            
    });
...
function tcbbkPattern($,instance){
    ...
    $('#ctl00_PlaceHolderEmptyMain_PlaceHolderMain_fecurrentid_gvResult tr').each(function (index) {
        ...
        if (index >= 1) {           
            ...
            if (index % 2 == 0) {
                key = result[0].trim()
                priceObj.bksell = (result[4] != undefined)? parseFloat(result[4].trim()).toFixed(5) : "-";
                priceObj.cashsell = (result[6] != undefined)? parseFloat(result[6].trim()).toFixed(5) : "-";
                instance.resultJson[key] = priceObj;
            }else{
                priceObj = {};
                priceObj.bkbuy = (result[4] != undefined)? parseFloat(result[4].trim()).toFixed(5) : "-";
                priceObj.cashbuy = (result[6] != undefined)? parseFloat(result[6].trim()).toFixed(5) : "-";
            }
...
function sinopacbkPattern($,instance){
    $('table table tr').each(function (index) {
        if (index >= 3) {       
            ...
            priceObj.bkbuy = parseFloat(result[1].trim()).toFixed(5);
            priceObj.bksell = parseFloat(result[2].trim()).toFixed(5);
            priceObj.cashbuy = (result[3] != undefined)? parseFloat(result[3].trim()).toFixed(5) : "-";
            priceObj.cashsell = (result[4] != undefined)? parseFloat(result[4].trim()).toFixed(5) : "-";
            instance.resultJson[key] = priceObj;
        }   
...
function kgibkPattern($,instance){
    $('.tb_05 tr').each(function (index) {
        if (index == 0) { 
            var time = $(this).text();
            instance.resultJson['time'] = time.match(/(\d+)(-|\/)(\d+)(?:-|\/)(?:(\d+)\s+(\d+):(\d+)(?::(\d+))?(?:\.(\d+))?)?/g);
...
function netbkPattern($,instance){
    $(".TitleForRate").siblings().each(function (index) {       
            ...
            if (key != "") {
                ...
                priceObj.cashbuy = (!isNaN(cashbuyTemp))?parseFloat(cashbuyTemp).toFixed(5):'-';
                priceObj.cashsell = (!isNaN(cashsellTemp))?parseFloat(cashsellTemp).toFixed(5):'-';
                instance.resultJson[key] = priceObj;
            }         
    });
